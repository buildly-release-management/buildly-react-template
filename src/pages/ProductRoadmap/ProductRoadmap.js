...
import { useThirdPartyToolSyncMutation } from '../../react-query/mutation/release/thirdPartyToolSyncMutation';
...
import { useDeleteFeatureMutation } from '../../react-query/mutation/release/deleteFeatureMutation';
import { useDeleteIssueMutation } from '../../react-query/mutation/release/deleteIssueMutation';
...
const ProductRoadmap = ({ history }) => {
  ...
  useEffect(() => {
    if (history && history.location && history.location.state) {
      setSelectedProduct(history.location.state.selected_product);
    } else if (!_.isEmpty(localStorage.getItem('activeProduct'))) {
      setSelectedProduct(localStorage.getItem('activeProduct'));
    } else {
  ...
  return (
    <>
      {(isAllProductLoading
        ...
        || isAllReleaseLoading
        || isBoardLoading
        || isThirdPartyToolSyncLoading
        || isCreatingFeatureLoading
        || isUpdatingProductLoading
        || isDeletingFeatureLoading
        || isDeletingIssueLoading)
        ? (
          <Loader open={isAllProductLoading
            || isAllCredentialLoading
            || isAllStatusLoading
            || isAllFeatureLoading
            || isAllIssueLoading
          ...
          <>
            {
              user.survey_status === true ? (
                <div className={classes.roadmapRoot}>
                  ...
                  {!_.isEmpty(selectedProduct) && _.toNumber(selectedProduct) !== 0 && !_.includes(_.uniq(_.map(statusData, 'product_uuid')), selectedProduct)
                    ? (
                      <>
                        ...
                        <Route
                          ...
                          render={(prps) => (
                            <Tabular
                              ...
                              upgrade={upgrade}
                              suggestedFeatures={product && product.product_info && product.product_info.suggestions}
                              statuses={statusData}
